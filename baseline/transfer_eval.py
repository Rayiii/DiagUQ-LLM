...
def test_transferability_mmlu(model_type, dataset_name):
    ...
    data_test_list, name_list, y_test, _ = load_MMLU_X_Y(
        phase="validation",
    ...
    output_dir = _TEST_OUTPUT_ROOT + "/MMLU/" + model_type + "/" + dataset_name + "/"
    if with_entropy:
...
def test_transferability(model_type, dataset_name):
    ...
    _, data_test_list, name_list, _, y_test, _, _ = load_X_Y(
    ...
    output_dir = _TEST_OUTPUT_ROOT + "/" + dataset_name + "/" + model_type + "/"
    if with_entropy:
        output_dir += "P2_with_entropy/"
    ...
    ood_output_dir = (
        _TEST_OUTPUT_ROOT + "/" + ood_dataset_name + "/" + model_type + "/"
    )
    if with_entropy:
        ood_output_dir += "P2_with_entropy/"
        if maintain_all_entropies:
